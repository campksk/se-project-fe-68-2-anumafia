        ...
        <div className="hidden md:flex flex-row items-center space-x-6 md:space-x-8">
          ...
          {session ? (
            <div className="flex items-center">
              ...
              <Link href="/signout" className="font-bold text-red-600 hover:text-red-700 transition-colors ml-6 bg-red-50 px-4 py-1.5 rounded-full">
                Sign-Out
              </Link>
            </div>
          ) : (
          ...
          {session ? (
            <div className="flex flex-col space-y-5">
              <Link href="/profile" onClick={closeMenu} className="font-semibold text-lg text-slate-800 hover:text-cyan-800 transition-colors">
                👤 {session.user?.name || "Profile Settings"} {isAdmin && <span className="text-sm text-gray-500">(Admin)</span>}
              </Link>
